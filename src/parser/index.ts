...
const processPage = (parserState: ParserState) => {
  ...
  page.startLine = parserState.currentLine;
  let finished = false
  let line = null
  ...
  page.endLine = parserState.currentLine
...
const processPlayer = (parserState: ParserState) => {
  ...
  let items = [];
  let startLine = parserState.currentLine;
  let endLine = -1;
  while (!finished && !parserState.isFinished()) {
    let line = parserState.getCurrentLine()
    if (!line.length) {
      ...
      if (tokens[0].value === 'PAGE') {
        ...
        let keyValues = getKeyValues(tokens)
        if (keyValues.ITEM === 'armor') {
          items.push({ type: keyValues.ITEM, name: keyValues.NAME, defense: keyValues.DEFENSE })
        } else if (keyValues.ITEM === 'weapon') {
          items.push({ type: keyValues.ITEM, name: keyValues.NAME, damage: keyValues.DAMAGE, speed: keyValues.SPEED })
        }
        parserState.nextLine()
      } else {
        let keyValues = getKeyValues(tokens)
  ...
  endLine = parserState.currentLine;
  const player = initalPlayer(name, attack, health, defense, currency)
  player.items = items;
  player.startLine = startLine;
  player.endLine = endLine;
